...
public class Util {
    ...
    public static List<Student> fillExaminations(List<Student> students) {
    	List<Examination> exams = Util.getExaminations();
    	return students.stream().peek(student -> student.setExamination(exams.stream().filter(exam->exam.getStudentId() == student.getId()).findFirst())).toList();
    }
